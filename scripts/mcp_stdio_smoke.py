...
async def run_smoke_test(project_directory: Path, call_timeout: int) -> int:
    ...
    try:
        ...
        call_unknown_tool = {
            ...
            "id": 3,
            "method": "tools/call",
            "params": {"name": "__non_existent_tool__", "arguments": {}},
        }
        process.stdin.write(json.dumps(call_unknown_tool) + "\n")
        process.stdin.flush()
        call_response = await _read_jsonrpc_response(process.stdout, timeout_seconds=20)
        if not isinstance(call_response, dict) or "id" not in call_response:
            raise RuntimeError(f"tools/call returned invalid response: {call_response!r}")
        if "result" not in call_response and "error" not in call_response:
            raise RuntimeError(f"tools/call missing result/error: {call_response!r}")
        print("PASS tools/call (received JSON-RPC response)")
...
async def run_interactive_call_test(
    project_directory: Path, call_timeout: int
) -> int:
    cmd = [
        "uv",
        "run",
    ...
    try:
        ...
        call_interactive_feedback = {
            ...
            "params": {
                ...
                "arguments": {
                    ...
                    "summary": "stdio smoke interactive call",
        ...
        _require_result(
            await _read_jsonrpc_response(
                process.stdout, timeout_seconds=max(45, call_timeout + 20)
            ),
            "tools/call interactive_feedback",
        )
        print("PASS interactive_feedback call")
        return 0
    except Exception as exc:
        print(f"Interactive call test failed: {exc}", file=sys.stderr)
        return 1
    finally:
        try:
            process.terminate()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Additionally run a real interactive_feedback tools/call check",
    )
...
def main() -> int:
    ...
    result = asyncio.run(run_smoke_test(project_directory, call_timeout=args.call_timeout))
    if result != 0:
        return result
    if args.exercise_interactive:
        return asyncio.run(
            run_interactive_call_test(project_directory, call_timeout=args.call_timeout)
        )
    return 0
